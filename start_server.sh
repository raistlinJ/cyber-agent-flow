#!/bin/bash
# Initialize the local environment and run the CyberAgentFlow SSH Server.
...
if [ "$BUILD_MODE" -eq 1 ]; then
    ...
    if ! "$PYTHON_BIN" -c "import requests, mcp, ollama, asyncssh" >/dev/null 2>&1; then
        echo "[cyber-agentflow] ERROR: Required Python dependencies are missing from $VENV_DIR"
        echo "[cyber-agentflow] Run ./start_server.sh --build once while online to install them."
        exit 1
    fi
fi
...
exec "$PYTHON_BIN" "$PROJECT_DIR/cli-server.py" "${FILTERED_ARGS[@]}"
